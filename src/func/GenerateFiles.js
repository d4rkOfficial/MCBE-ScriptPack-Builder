 ...
 * @param {string} selected_v_mc - The selected minecraft/server version.
 * @param {string} selected_v_ui - The selected minecraft/server-ui version.
 * @param {string} selected_v_test - The selected minecraft/server-gametest version.
 * @param {string} selected_v_data - The selected minecraft/vanilla-data version.
 * @returns {Promise<void>} A promise that resolves when all files are generated.
 */
export default async function generateFiles(packName, manifestRes, manifestBeh, selected_v_mc, selected_v_ui, selected_v_test, selected_v_data) {
    ...
    try {
        ...
        packIcon.data = [Math.floor(Math.random() * 256), Math.floor(Math.random() * 256), Math.floor(Math.random() * 256), 255]
        await fs.writeFile(path.join(projectPath, "resource_packs", packName, "pack_icon.png"), PNG.sync.write(packIcon))
        await fs.writeFile(path.join(projectPath, "behavior_packs", packName, "pack_icon.png"), PNG.sync.write(packIcon))

        await fs.writeFile(path.join(projectPath, "resource_packs", packName, "manifest.json"), JSON.stringify(manifestRes, null, 4))
        await fs.writeFile(path.join(projectPath, "behavior_packs", packName, "manifest.json"), JSON.stringify(manifestBeh, null, 4))

        await fs.writeFile(
            path.join(projectPath, "scripts", "main.ts"),
            dedent`
        ...
        await fs.writeFile(
            path.join(projectPath, "just.config.ts"),
            dedent`
        ...
        await fs.writeFile(
            ...
            dedent`
            {
                ...
                "dependencies": {
                    ...
                    "@minecraft/server-gametest": "${selected_v_test}",
